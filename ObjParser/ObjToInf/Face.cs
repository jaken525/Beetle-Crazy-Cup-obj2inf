...
namespace ObjParser.Types
{
    public class Face : IType
    {
        ...
        public void LoadFromStringArray(string[] data)
        {
            ...
            for (int i = 0; i < vcount; i++)
            {
                ...
                if (parts.Count() > 1)
                {
                    ...
                    if (success) TextureVertexIndexList[i] = vindex;
                }
            }
        }
        ...
        public override string ToString()
        {
            ...
            b.AppendFormat("{0}/{1}", VertexIndexList[0], TextureVertexIndexList[0]);

            for (int i = 1; i < VertexIndexList.Count(); i++)
                if (i < TextureVertexIndexList.Length)
                    b.AppendFormat("\n{0}/{1}", VertexIndexList[i], TextureVertexIndexList[i]);
                else
